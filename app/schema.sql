...
CREATE TRIGGER IF NOT EXISTS update_action_type_after_insert
    AFTER INSERT ON AUDIT
    FOR EACH ROW
    WHEN NEW.ACTION_TYPE IS NULL AND
        (NEW.ENTITY_ID LIKE 'CREATED NEW CATEGORY%' OR NEW.ENTITY_ID LIKE 'EDITED ITEM%')
    BEGIN
        UPDATE AUDIT
        SET ACTION_TYPE = NEW.ENTITY_ID
        WHERE ID = NEW.ID;
    END;
